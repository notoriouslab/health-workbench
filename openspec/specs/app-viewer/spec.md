...
選中項目的趨勢圖（含參考值呈現——灰帶、單條參考線或不畫，依
「檢驗參考值的解析與呈現」判定——與 knowledge 說明，行為沿用原趨勢頁
檢驗段）與該項逐筆表。文字型結果的項目 MUST 照常列於清單並顯示逐筆表、
僅不繪圖（沿用既有語意）。

總覽「最新檢驗」表與搜尋結果的檢驗項目點入 MUST 跳轉至檢驗分頁並選中
該項目，MUST NOT 再指向已不存在的趨勢分頁。

分頁 SHALL 以一行說明本頁資料來自醫療院所的檢驗結果（見「匯出檔的收檔端
可辨識與替代路徑」的分頁來源條款）。
...
#### Scenario: 分頁標示資料來源
- **WHEN** 使用者進入檢驗分頁
- **THEN** 頁內有一行說明這頁是醫療院所的檢驗結果

<!-- @trace
source: clinic-visit-view
updated: 2026-08-21
code:
  - docs/verification/display_revamp.md
  - docs/verification/drug_info_refband.md
  - docs/verification/clinic_visit_view.md
-->
...
分頁 SHALL 以一行說明本頁資料來自使用者自己的裝置與健檢量測（見「匯出檔的
收檔端可辨識與替代路徑」的分頁來源條款）。
...
---
### Requirement: 用藥卡的適應症顯示

用藥分頁的用藥卡展開區 SHALL 於成分之下顯示官方登記適應症原文，
標示「官方登記適應症原文」與許可證資料更新日期；適應症原文屬
引述原文，不受非結論式用語約束，但 UI 標籤與註記文字本身 MUST
通過禁用詞檢查。超過約 120 字 SHALL 截斷並提供「顯示全部」切換。
用法用量有值時 SHALL 於適應症下方附原文段，無值 MUST NOT 留空段。
許可證狀態非有效者（如已註銷、已廢止）SHALL 以中性語氣註記原狀態
（歷史處方指向非有效證為常態，MUST NOT 用警示語氣）；許可證 join
未命中或快取無適應症欄位者
MUST NOT 顯示適應症區塊，既有「品項檔未對照」語意不變。

#### Scenario: 適應症呈現
- **WHEN** 展開一筆許可證 join 命中且有適應症的用藥卡
- **THEN** 見「官方登記適應症原文」標示、資料更新日期與適應症
  文字；超過約 120 字時先見截斷版並可展開全文

#### Scenario: 已註銷許可證
- **WHEN** 展開一筆許可證已註銷的用藥卡
- **THEN** 適應症照常顯示，並見中性註記說明許可證已註銷

#### Scenario: 舊快取無適應症欄位
- **WHEN** 以缺少適應症欄位的舊快取檔產出的檢視頁展開用藥卡
- **THEN** 卡片正常顯示商品名與成分，無適應症區塊、無錯誤
...
用藥分頁 SHALL 提供「列印用藥清單」：以列印視圖（藥品與中醫用藥
兩節，每節內 SHALL 再分「目前在吃」與「過往」兩區；每列含名稱、成分、
最近處方日期，「目前在吃」區另含剩餘天數；頁首含成員名、產生日期、
資料截止日與品項檔版本來源標示；頁尾標示清單僅為就醫溝通輔助）呼叫系統列印
對話框，由使用者自行選擇印出或存為 PDF。列印 MUST NOT 連網、
MUST NOT 由程式自行產出檔案。列印時 MUST 只呈現清單視圖、不含
應用程式介面其他部分。診療項目類 MUST NOT 進入清單。

「目前在吃」與「過往」的判定 SHALL 與診間視角區塊採用同一實作，MUST NOT
另寫一份判定邏輯。先前版本未區分現用與過往，使用者會把早已停用的藥一併
帶進就醫溝通場合。

列印按鈕 SHALL 於瀏覽器頂層視窗（匯出之單檔 HTML）提供；App 內
檢視（內嵌框架，系統列印不作用，2026-08-20 實機驗證）MUST NOT
顯示無作用按鈕，SHALL 改顯示導流說明（先匯出單檔 HTML、於瀏覽器
列印）。EPUB 匯出 MUST NOT 顯示列印按鈕與導流說明（閱讀器的列印
語意不受控）。

#### Scenario: 列印清單內容
- **WHEN** 於瀏覽器開啟的檢視頁按「列印用藥清單」
- **THEN** 系統列印對話框開啟，預覽僅含清單視圖：藥品與中醫用藥
  分節、各節內分「目前在吃」與「過往」、每列名稱＋成分＋最近處方日期、
  頁首來源標示與資料截止日

#### Scenario: 現用與過往分區
- **WHEN** 使用者有一款給藥期間未過期的藥與一款兩年前開立的藥
- **THEN** 前者列於「目前在吃」並顯示剩餘天數，後者列於「過往」

#### Scenario: App 內導流
- **WHEN** 於 App 內檢視頁開啟用藥分頁
- **THEN** 不見列印按鈕，見導流說明（先匯出單檔 HTML 再於瀏覽器
  列印）

#### Scenario: 匯出 HTML 的列印
- **WHEN** 於瀏覽器開啟匯出的單檔 HTML 並按「列印用藥清單」
- **THEN** 瀏覽器列印對話框開啟，預覽同上，可另存 PDF

#### Scenario: EPUB 不提供列印
- **WHEN** 於 EPUB 閱讀器開啟匯出的 EPUB 檢視用藥分頁
- **THEN** 不見列印按鈕，其餘互動不受影響
...
---
### Requirement: 檢驗參考值的解析與呈現

檢驗趨勢圖的參考值解析 SHALL 支援三種形態：雙欄格式
（`[低][高]`，一側為「無」或非數值時視為一側性）、範圍格式
（`低-高`／`低~高`）、一側性符號格式（`<`／`≦`／`≤`／`以下` 為
上限，`>`／`≧`／`≥`／`以上` 為下限）。雙側有值 SHALL 呈現參考值
灰帶；一側性 SHALL 呈現單條參考線並標示上限或下限，MUST NOT 以
帶呈現；說明文字 SHALL 隨形態切換。

含括號字串的判定順序 SHALL 為：先試雙欄格式（全字串恰為
`[左][右]` 兩組）；未匹配者再試單一括號組（全字串恰為 `[內容]`
一組），內容為單一範圍（`[0-40]`）或可判方向之一側形（`[90-]`＝
下限、`[-40]`＝上限）者 SHALL 解析（範圍呈現灰帶、一側形呈現
單條參考線，與其他形態的呈現規則相同；裸 `90-` 這類無括號無符號
的一側寫法不在支援範圍），
內容為無方向單數或無數值者不呈現；**雙者皆未匹配的含括號字串**
（多個獨立括號組之年齡分段複合參考值與一切未知括號形）MUST NOT
解析、MUST NOT 呈現任何參考帶或參考線（保守不畫，取代錯誤解析）。
兩側皆無數值者同樣不呈現。解析測試向量 MUST 取自真實匯入資料的
參考值格式。

#### Scenario: 一側性參考值
- **WHEN** 檢視參考值為「<150」（或 `[無][150]`）之檢驗項目趨勢
- **THEN** 圖上呈現單條參考上限線與其標示，無灰帶

#### Scenario: 雙欄格式參考值
- **WHEN** 檢視參考值為 `[5.0][24.0]` 之檢驗項目趨勢
- **THEN** 圖上呈現 5.0-24.0 參考值灰帶

#### Scenario: 無括號範圍格式
- **WHEN** 檢視參考值為 `35-160` 之檢驗項目趨勢
- **THEN** 圖上呈現 35-160 參考值灰帶

#### Scenario: 單括號組參考值
- **WHEN** 檢視參考值為 `[0-40]`（或 `[90-]`）之檢驗項目趨勢
- **THEN** `[0-40]` 呈現 0-40 灰帶；`[90-]` 呈現參考下限線

#### Scenario: 複合字串保守不畫
- **WHEN** 檢視參考值為年齡分段複合字串之檢驗項目趨勢
- **THEN** 不呈現任何參考帶或參考線，趨勢圖其餘照常

<!-- @trace
source: prerelease-p0-fixes
updated: 2026-08-20
code:
  - docs/verification/drug_info_refband.md
  - docs/verification/prerelease_p0_fixes.md
-->

---
### Requirement: 診間視角區塊

就醫分頁 SHALL 於篩選器**之上**提供固定區塊，內容為就醫溝通時最常被詢問的
項目。該區塊 MUST NOT 受類型與院所篩選影響（篩選服務於下方歷史瀏覽，與本
區塊的「目前狀態」語意正交）。下方年份分組清單、篩選器與逐筆展開的行為
MUST NOT 因本區塊而改變。

區塊 SHALL 含四個小節：近日用藥、居家量測、檢驗、資料裡的變化。有資料的
小節才渲染；無資料的小節 MUST 整段不輸出（MUST NOT 留空標題或空表格）；
四節全空時整個區塊 MUST NOT 渲染。區塊底部 SHALL 提供補充資料的導引。

小節標題 MUST NOT 宣稱超出資料所能支持的事實（2026-08-21 走查定案）：
用藥小節標為「近日用藥」而非問句式的「現在在吃什麼藥」，因為判定依據是
給藥日數推算，不是服藥事實。項目數可能偏多的小節（檢驗）SHALL 可折疊，
預設收合並在摘要行標示日期與項目數。

區塊本身 MUST NOT 另立大標題：它已置於分頁頂端，標題只是多一行佔掉診間
十秒的視覺預算；資料截止日那一行即區塊的標頭。列印視圖（摘要卡）不受此
限制，列印出來的紙需要自己的標題。

本區塊 MUST NOT 新增 payload 欄位，資料一律取自既有 payload。

#### Scenario: 篩選不影響區塊
- **WHEN** 使用者在就醫分頁選擇類型「牙科」
- **THEN** 下方清單只剩牙科就醫，上方區塊內容完全不變

#### Scenario: 只匯入健保資料的使用者
- **WHEN** 資料庫無 Apple Health 與 CPAP 紀錄
- **THEN** 居家量測小節整段不出現（無空標題、無空表格），區塊底部出現
  補充資料的導引

#### Scenario: 空資料庫
- **WHEN** 四個小節皆無可呈現內容
- **THEN** 整個區塊不渲染，就醫分頁外觀與本變更前一致
...
---
### Requirement: 目前在吃的藥的判定

「目前在吃」SHALL 判定為「就醫日期加給藥日數尚未過期」，且分類為藥品或
中醫用藥（診療項目類 MUST NOT 納入）。給藥日數取自官方欄位（r1_1.4／
r3_1.6「給藥日數」），屬資料自帶事實。

給藥日數為空或零者 MUST NOT 判為目前在吃，SHALL 歸入過往並標註無給藥日數。
MUST NOT 以任何預設天數補值。判定 MUST NOT 加入寬容期。

慢性病連續處方箋 MUST NOT 特別處理：分次領藥各自為獨立紀錄並各帶給藥日數，
起算日由既有「藥局交付調劑日期回退」條文保證。

用藥小節 SHALL 附註自費藥品與保健食品不在本資料範圍內。該註記 MUST 僅陳述
本 App 的資料範圍，MUST NOT 述及其他系統或醫療人員可取得的資訊。

#### Scenario: 給藥期間未過期
- **WHEN** 某藥最近一次處方為 30 日前、給藥日數 28 日
- **THEN** 該藥不在「目前在吃」，列於過往

#### Scenario: 給藥日數缺值
- **WHEN** 某藥品項的給藥日數為空
- **THEN** 該項列於過往並標註無給藥日數，MUST NOT 出現在「目前在吃」

#### Scenario: 短期用藥不誤判
- **WHEN** 三日份藥品於 60 日前開立，且為該藥最近一次處方
- **THEN** 不判為目前在吃
...
---
### Requirement: 居家量測小節的指標集

居家量測小節 SHALL 自固定候選集取用有資料者：血壓（收縮與舒張）、體重、
睡眠（含睡眠呼吸治療的 AHI 與使用時數）。MUST NOT 列出候選集以外的量測
型別（避免本區塊退化為第二個總覽頁）。

每項 SHALL 同時呈現近 7 日與近 30 日的代表值與**有量測的天數**。代表值
SHALL 為該窗格內每日值的中位數。

可信度指標 MUST 為天數而非量測次數：payload 的量測序列已是每日中位數、
不含原始筆數，寫成「次數」是憑空造出資料層沒有的精度。天數 MUST 呈現：
兩天有量與二十八天有量的中位數可信度不同，僅呈現數值會讓兩者無從分辨。

#### Scenario: 呈現有量測的天數
- **WHEN** 候選集中的某個量測項目，近 7 日內有 3 天有紀錄、近 30 日內有
  22 天有紀錄
- **THEN** 兩個窗格各自顯示中位數與其天數，且措辭為天數而非次數
...
---
### Requirement: 資料裡的變化（自動生成、唯讀）

小節 SHALL 以純事實陳述呈現，MUST NOT 含建議動詞（「應該」「建議」「需要」
「請」等），MUST NOT 構成醫療判斷。節內 SHALL 標示既有免責語（清單僅為
就醫溝通輔助，不含醫療判斷）。本小節 MUST 為唯讀，本變更 MUST NOT 引入
使用者輸入。

規則集 SHALL 限於下列兩條：

1. **連續三次超出參考值**：某檢驗項目最近三次結果皆落在該項目參考值之外。
   參考值解析 MUST 重用既有的參考值解析實作（MUST NOT 複製第二份實作）；
   參考值無法解析者（年齡分段複合形等）MUST NOT 納入判定。「連續三次」為
   本設計選定的呈現門檻，非醫學判準。
2. **距上次檢驗最久**：列出距上次檢驗最久的前三個項目，僅呈現項目名與上次
   日期。此規則 MUST NOT 使用任何絕對天數門檻。僅納入歷史上至少出現兩次的
   項目（只驗過一次者不構成「很久沒驗」）。

#### Scenario: 連續三次超標
- **WHEN** 某項目最近三次結果皆高於其參考值上限且參考值可解析
- **THEN** 該項目出現於本小節，措辭為事實陳述且不含建議動詞

#### Scenario: 參考值無法解析時保守不報
- **WHEN** 某項目參考值為年齡分段複合形（既有條文明定不解析）
- **THEN** 該項目 MUST NOT 出現於本小節，即使數值明顯偏離

#### Scenario: 距上次最久不用絕對門檻
- **WHEN** 使用者所有檢驗項目最近一次都在三個月內
- **THEN** 仍列出其中距今最久的前三項（相對排序，不因未達任何天數門檻而空白）
...
---
### Requirement: 區塊的資料截止日

區塊標頭 SHALL 標示資料截止日（取自既有 payload 的資料期間上界）。截止日
距今超過三個月時 SHALL 追加提示，告知較新的就醫不在此範圍內。三個月為本
設計選定的呈現門檻，非醫學判準。

該上界取自就醫紀錄（`MAX(date) FROM encounters`），**不涵蓋居家量測**：
Apple 或呼吸器資料可能比它新。因此措辭 MUST 明指是就醫資料的截止日，
MUST NOT 只寫「資料截止」——那會讓使用者以為整份資料都停在那一天。

理由：「目前在吃的藥」的正確性完全依賴資料新鮮度。使用者若持半年前的資料
就醫，沉默地呈現一份過期清單比不呈現更糟。

#### Scenario: 資料新鮮
- **WHEN** 資料截止日距今一個月
- **THEN** 標頭顯示截止日，不顯示過期提示
...
---
### Requirement: 看診摘要卡的列印視圖

檢視頁 SHALL 另提供「看診摘要卡」列印視圖，內容為診間視角區塊的精簡
一頁式版本。其按鈕顯示條件與既有列印清單一致（瀏覽器頂層提供、App 內顯示
導流說明、EPUB 不顯示），**按鈕樣式與文案 SHALL 與既有列印按鈕同一形式**
（`列印<對象>` 加同一個按鈕類別）：兩顆做同一件事的按鈕長得不一樣，使用者
要多花一次判斷。

摘要卡 SHALL 涵蓋區塊的**四個小節**：近日用藥、居家量測、最近一次抽血、
資料裡的變化。「精簡」指的是每節取精簡形（例如用藥不重複列最近處方日期），
MUST NOT 整節省略——「上次抽血結果如何」是看診時最常被問的一句，缺了它
摘要卡就少掉主要用途之一（2026-08-21 走查發現首版漏列）。

摘要卡內 MUST NOT 使用折疊區：列印出來的紙沒有「展開」這個動作，收合的
內容等於沒印。畫面上的區塊可以折疊（見「診間視角區塊」的折疊條款），
兩者的載體不同，規則不共用。

多份列印視圖並存時，列印 MUST 只呈現被觸發的那一份。實作 SHALL 以作用中
標記區分，MUST NOT 讓兩份視圖同時進入列印輸出。

#### Scenario: 只印被觸發的那一份
- **WHEN** 使用者按「列印看診摘要卡」
- **THEN** 列印預覽僅含摘要卡，MUST NOT 含用藥清單視圖

#### Scenario: 反向亦然
- **WHEN** 使用者按「列印用藥清單」
- **THEN** 列印預覽僅含用藥清單，MUST NOT 含摘要卡
...
---
### Requirement: 使用者可見文案的比較性措辭約束

使用者可見文案 MUST NOT 述及其他醫療資訊系統的資料涵蓋範圍，MUST NOT 聲稱
或暗示醫療人員無法取得某些資料，MUST NOT 出現其他系統的收載區間數字。此類
資訊 MUST NOT 寫入程式常數或資料檔。

理由：使用者可能因此誤判醫療人員手上資訊的完整性，或誤認本 App 的資料更
具權威性。本 App 的資料同源且新鮮度較低，此類比較性敘述有實質誤導風險。

本約束 SHALL 由測試守衛檢查，且守衛 MUST 有負向對照（故意寫入違規字串時
測試轉紅），MUST NOT 僅驗證現況為綠。

#### Scenario: 守衛擋下比較性措辭
- **WHEN** 檢視層字串出現醫療人員與「查不到」同句共現、或出現其他系統的
  收載區間數字
- **THEN** 守衛測試失敗並指出違規字串

#### Scenario: 守衛自身有效
- **WHEN** 在檢視層故意寫入一句違規措辭
- **THEN** 守衛測試轉紅（證明其真的會 fire，而非恆綠）
...
---
### Requirement: 匯出檔的收檔端可辨識與替代路徑

以下三條的對象都是**收到檔案的人**：他手上只有那一顆檔案，README 不會跟著
檔案走（來源：docs/spikes/20260820_ux_deep_review.md 的 friction 2／11／16）。

**檔名** SHALL 以中文詞起頭並含成員名與日期（`健康紀錄_{成員}_{YYYYMMDD}-private`）。
理由：檔案會躺在通訊軟體或郵件的附件列表裡，英文詞起頭的檔名讓收檔的人認不出
這是什麼。`-private` 後綴 SHALL 保留（給寄件人自己的提醒）。HTML 與 EPUB
兩條匯出路徑 MUST 共用同一套命名規則。

**JS 未執行時的說明** SHALL 同時給出兩條替代路徑：改用會執行網頁程式的
檢視器，**以及請寄件人改寄 EPUB 版**。先前只寫前者，而後者對收檔端更省事
（不必安裝任何東西），卻只寫在 README 給寄件人看。單檔 HTML 與 EPUB 兩端
的這段說明 MUST 同步（兩份產物給同一個人看，說法不得不一致）。

**分頁的資料來源** SHALL 各以一行說明：檢驗來自醫療院所、測量來自使用者
自己的裝置與健檢。分頁的分界是資料來源的內部邏輯，不是使用者的心智模型。

#### Scenario: 收檔端在附件列表認出檔案
- **WHEN** 對成員「媽媽」於 2026-08-10 匯出單檔 HTML
- **THEN** 檔名為 `健康紀錄_媽媽_20260810-private.html`

#### Scenario: 預覽視窗打不開時看得到正解
- **WHEN** 以不執行網頁程式的方式開啟匯出的單檔 HTML
- **THEN** 畫面上的說明同時提到「用瀏覽器或檢視器 App 開啟」與「請寄件人
  改寄 EPUB 版」
