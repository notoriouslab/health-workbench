...
const SAFE_FRAGMENTS = [
  ...
  // 官方標準常數字面（非個人量測值）：只豁免常數本身，NEVER 用 .* 跨距
  // 豁免——那會把夾在中間的其他內容一起消掉，等於在守衛上開洞
  [/130\s*[／\/]\s*80|18\.5\s*[-~─—]\s*24/g, "血壓居家判準與 BMI 參考帶（2022 台灣高血壓治療指引、國民健康署發布的官方標準常數）"],
  [/2022\s*台灣高血壓治療指引/g, "文獻名（年份屬名稱的一部分）"],
  [/722\s*原則/g, "國民健康署居家血壓量測原則的官方名稱（7 天 2 次 2 遍）"],
  [/收縮\s*130[、\s]*[與]?\s*舒張\s*80/g, "血壓居家判準的展開寫法（同 130／80 官方常數）"],
  [/\b(9|20|29)\s*個型別/g, "型別分配表的清單計數（規格常數，非量測值）"],
  [/值域(?:為)?\s*0-1|0-1\s*標\s*%/g, "HealthKit 官方值域定義（血氧、行走穩定度存 0-1）"],
  [/取\s*[12]\s*位/g, "SQL ROUND 捨入位數（設計常數）"],
  [/「<150」|\[無\]\[150\]/g,
    "檢驗參考值格式的規格範例（三酸甘油酯通用參考上限，醫界標準常數，適用所有人，非任何個人的檢驗值）"],
  [/dataset\/9122/g, "政府開放資料集編號（藥品許可證資料集的公開識別碼，非量測值）"],
  [/\b\d{5}[A-Z]\b/g, "健保醫令代碼（5 位數字加 1 大寫字母，TW Core 公開 CodeSystem 的常數識別碼，非任何人的量測值）"],
  [/`35-160`/g, "檢驗參考值格式的規格範例（反引號內的格式示例常數，非任何個人的檢驗值）"],
  [/近\s*(?:7|30)\s*天/g,
    "診間視角的窗格長度（design D3 選定的呈現窗格，是設計常數而非任何人的量測值）"],
];
